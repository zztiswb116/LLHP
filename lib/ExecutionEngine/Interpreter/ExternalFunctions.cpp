...
static bool ffiInvoke(RawFunc Fn, Function *F, ArrayRef<GenericValue> ArgVals,
  ...
  if (ArgVals.size() > NumArgs && F->isVarArg()) {
    Type *VarArgTy = FTy->getParamType(FTy->getNumParams()-1);//Get Last Type
    for(size_t i=0;i<ArgVals.size()-NumArgs;i++){
      //Start mapping the remaining arguments's Types
      args[i+NumArgs] = ffiTypeFor(VarArgTy);
      ArgBytes += TD.getTypeStoreSize(VarArgTy);
    }
  }
  ...
  if (ArgVals.size() > NumArgs && F->isVarArg()) {
    Type *VarArgTy = FTy->getParamType(FTy->getNumParams()-1);//Get Last Type
    for(size_t i=0;i<ArgVals.size()-NumArgs;i++){
      //Start mapping the remaining arguments's Values
      values[i+NumArgs] = ffiValueFor(VarArgTy, ArgVals[i+NumArgs], ArgDataPtr);
      ArgDataPtr += TD.getTypeStoreSize(VarArgTy);
    }
  }
